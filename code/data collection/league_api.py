...
class LeagueAPI():
    ...
    def get_player_name(self,summoner_name):
        url = "{}/summoner/v4/summoners/by-name/{}".format(self.base_url,summoner_name)
        data = requests.get(url=url,params = self.params)
        time.sleep(1.3)
        if data.status_code == 200:
            player_list = []
            data_json = json.loads(data.text)
            return (data_json['name'],data_json['id'],data_json['accountId'])
        elif data.status_code == 403: #need to reset API
            self.reset_key()
            self.get_player_name(summoner_name)
